...
PAAC is trained with a joint objective that combines the standard BPR recommendation loss with two additional components:

**Supervised Alignment.** For each user, items in their interaction history are split into a popular group and an unpopular group based on training-set item frequency. Representations of items in the two groups are pulled together, transferring the richer supervision available for popular items to their unpopular counterparts.

**Re-weighting Contrast.** Within each mini-batch, items are dynamically split into popular/unpopular groups (top-x% vs. the rest). Two hyperparameters control the relative weight given to popular vs. unpopular items when they act as positive and negative samples in the contrastive loss — preventing the optimization from over-separating the two groups.

PAAC introduces **no additional trainable parameters** over the backbone encoder. For the full derivation, empirical motivation, and algorithm listing, please see the [paper](https://arxiv.org/abs/2405.20718) (Sections 2–3, Algorithm 1).
...
- National Key Research and Development Program of China (Grant No. 2021ZD0111802)
- National Natural Science Foundation of China (Grant No. 72188101, U21B2026)
...
For questions about the paper or code, please open a [GitHub issue](https://github.com/miaomiao-cai2/KDD2024-PAAC/issues) or reach out via the author emails listed in the [paper](https://arxiv.org/abs/2405.20718).
